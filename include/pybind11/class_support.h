...
/// C++ object (or multiple objects, for Python-side inheritance from multiple pybind11
/// types), but doesn't call the constructor -- an `__init__` function must do that.
...
extern "C" inline void pybind11_object_dealloc(PyObject *self) {
    ...
    for (auto &v_h : values_and_holders(instance)) {
        if (v_h) {
            ...
            auto range = registered_instances.equal_range(v_h.value_ptr());
            bool found = false;
