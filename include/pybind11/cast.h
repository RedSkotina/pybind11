...
public:
    ...
    PYBIND11_NOINLINE static handle cast(const void *_src, return_value_policy policy, handle parent,
        ...
        bool owned = false, apply_keepalive = false;
        ...
        // Delay type allocation and initialization until after the above so that, if the above
        // throws, we don't leave a partially-initialized instance around.
        //
        // Also initialize the object manually (rather than calling new): we don't want an allocated
        // and initialized value pointer initialized because we already have a pointer, from above,
        // to set it to.
        auto inst = reinterpret_steal<object>(PyType_GenericAlloc(tinfo->type, 0));
        auto wrapper = reinterpret_cast<instance *>(inst.ptr());
        wrapper->allocate_layout();
        wrapper->owned = owned;

        auto vhs = values_and_holders(wrapper);
        vhs.begin()->value_ptr() = valueptr;
        ...
        internals.registered_instances.emplace(valueptr, wrapper);
